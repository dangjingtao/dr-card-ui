...
- 状态：`Agent Review`（2026-09-11 实施完成，本地工程门与 375×812 取证通过，已过一轮 PRD 检查并修订条款，等用户验收；按全局契约只有用户可以标 Accepted）
- 类型：Bug Fix（路由跳转错误）
- 优先级：P0（影响品牌专栏核心入口语义）
...
- ⚠️ 阻塞验收：`## 必交证据` 的「对应提交号」尚未回填，本卡**不具备进入 `User Review` 的条件**。
...
| **对应提交号** | ❌ **待回填**（提交后补齐；在此之前本卡不具备进入 `User Review` 的条件） |
