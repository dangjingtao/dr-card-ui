...
export const ROUTES: RouteMeta[] = [
  ...
  {
    path: '/legacy-profile/my-cards/:id/edit/:field',
    title: '编辑卡字段',
    titleBar: 'hidden',
    nodes: [],
    task: 'T031',
    entry: '卡详情-修改（姓名/班级/学号）',
    returnTo: '卡详情',
    owner: '卡字段编辑页（T031 补充；顶部白色标题栏 + 输入框 + 底部淡金渐变保存按钮）',
  },
  {
  ...
  },
  {
    ...
    entry: '学校账户-购买',
    returnTo: '学校账户',
    owner: '充值/购买页（T028；学校名头部 + 购买金额输入 + 充20/50元 chip + 支付金额 + 蓝色确认提交 + 绿色退款 + 温馨提示 + 广告位）',
  },
  {
    ...
    nodes: [],
    task: 'T028',
    entry: '学校账户-退款 / 充值页-退款',
    returnTo: '学校账户',
    owner: '学校退款页（T028；退款金额输入（默认=可退款金额）+ 退款方式占位 + 灰色确认退款 + 温馨提示）',
  },
  ...
  /* ────────────────────────── T033/T034 卡博士设备服务 ────────────────────────── */
  {
    ...
    nodes: [],
    task: 'T034',
    entry: '卡博士首页-四宫格（淋浴/洗烘/饮水/吹风）',
    returnTo: '卡博士首页',
    owner: '设备列表页（T034；四套主题色通过 :type 参数切换）',
  },
  {
    ...
    nodes: [],
    task: 'T034',
    entry: '设备列表-扫码使用；扫码页扫码成功后',
    returnTo: '设备列表',
    owner: '设备详情页（T034；金额选择 + 确定启动 + 启动动画 + 结算 + 紧急停止）',
  },
  {
    path: '/device/success',
    title: '设备详情',
    ...
    nodes: [],
    task: 'T034',
    entry: '（旧入口，重定向到 /device/connecting）',
    returnTo: '设备列表',
    owner: '设备详情页（T034；同 /device/connecting）',
  },
  {
    path: '/card',
    title: '卡包',
