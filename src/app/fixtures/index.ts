...
/**
 * T043｜诗得丽专栏入口 - 卡博士存量用户占位券演示态。
 *
 * 触发条件：当前账号 `isDearseedCare === false`（不属于诗得丽关爱机项目）。
 * 占位券的具体券种未定（B-044），本期先以「通用兑换券」演示位呈现，等丁总指定
 * 券种后只需替换 fixture 文案/图标，组件无需改动。
 */
export const PLACEHOLDER_COUPON = {
  /** 顶部 tag：演示态标识，醒目但不刺眼 */
  demoTag: '演示位 · 后续接入',
  ...
  desc: '正式券种由丁总确认后接入。',
  /** 主按钮文案 */
  action: '了解卡博士存量用户专属兑换',
  /** 主按钮跳转目标：当前指向 /mall（H5 商城），券种确认后可切换为占位详情页 */
  actionTo: '/mall',
  /** 关闭按钮 aria-label */
  dismissLabel: '关闭占位券演示',
} as const
